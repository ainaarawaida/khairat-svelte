<script>
  ...
  const submitHandler = async (e) => {
    ...
    if (errors.length === 0) {
      ...
      dataArray.append("action", "updateProfil");
  ...
  onMount(async () => {
    ...
    let year = fields.user_registered.substring(0, 4);
    let month = fields.user_registered.substring(5, 7);
    let day = fields.user_registered.substring(8, 10);
...
<div class="container-fluid px-4">
  <h3 class="mt-4">Profil Pengguna</h3>
      ...
      <div class="col">
        ...
        >
          ...
          <input
            ...
            type="text"
            class="woocommerce-Input woocommerce-Input--text input-text form-control {errors.some(
              (d) => d.key.includes('jenis_ahli')
            )
              ? 'is-invalid'
              : ''}"
            name="jenis_ahli"
            id="jenis_ahli"
            bind:value={fields.jenis_ahli_x}
            required
          />
